 ...
 *   Last modification: 25 November 2022
 */
   ...
   MPI_Comm grid_comm)   /* IN - Communicator */
{
   ...
   if (grid_id == 0) {
      ...
      if (infileptr == NULL) *m = 0;
         ...
         dummy = fread (m, sizeof(int), 1, infileptr);
         dummy = fread (n, sizeof(int), 1, infileptr);
      }
   }
   ...
   *subs = (void **) my_malloc (grid_id,local_rows*PTR_SIZE);
   ...
   for (i = 0; i < grid_size[0]; i++) {
      ...
      for (j = 0; j < BLOCK_SIZE(i,grid_size[0],*m); j++) {
         ...
         if (grid_id == 0) {
            dummy = fread (buffer, datum_size, *n, infileptr);
         }
   ...
   MPI_Comm     comm)     /* IN - Communicator */
{
   ...
   *subs = (void **) my_malloc (id, local_rows * PTR_SIZE);
   ...
   if (id == (p-1)) {
      for (i = 0; i < p-1; i++) {
         dummy = fread (*storage, datum_size,
            BLOCK_SIZE(i,p,*m) * *n, infileptr);
         MPI_Send (*storage, BLOCK_SIZE(i,p,*m) * *n, dtype,
            i, DATA_MSG, comm);
      }
      dummy = fread (*storage, datum_size, local_rows * *n,
         infileptr);
      fclose (infileptr);
   } else
    ...
    MPI_Comm     comm)   /* IN - Communicator */
{
   ...
   if (id == (p-1)) {
      ...
      else dummy = fread (n, sizeof(int), 1, infileptr);
   }
   MPI_Bcast (n, 1, MPI_INT, p-1, comm);
   if (! *n) {
      if (!id) {
   ...
   *v = my_malloc (id, local_els * datum_size);
   if (id == (p-1)) {
      for (i = 0; i < p-1; i++) {
         dummy = fread (*v, datum_size, BLOCK_SIZE(i,p,*n),
            infileptr);
         MPI_Send (*v, BLOCK_SIZE(i,p,*n), dtype, i, DATA_MSG,
            comm);
      }
      dummy = fread (*v, datum_size, BLOCK_SIZE(id,p,*n),
             infileptr);
      fclose (infileptr);
   } else {
   ...
   MPI_Comm     comm)   /* IN - Communicator */
{
   ...
   *v = my_malloc (id, *n * datum_size);
   ...
   if (id == (p-1)) {
      dummy = fread (*v, datum_size, *n, infileptr);
      fclose (infileptr);
   }
...
void print_checkerboard_matrix (
   ...
   MPI_Comm     grid_comm)    /* IN - Communicator */
{
   void      *buffer=NULL;    /* Room to hold 1 matrix row */
   ...
   MPI_Comm     comm)    /* IN - Communicator */
{
   //MPI_Status status;     /* Result of receive */
   int        datum_size; /* Bytes per matrix element */
   void      *buffer=NULL;/* Enough room to hold 1 row */
   int        i;
   int        id;         /* Process rank */
   int        p;          /* Number of processes */
